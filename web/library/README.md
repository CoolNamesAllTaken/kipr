# Library review viewer

Static web app that shows the new/changed KiCad footprints and symbols of a PR. Plain ES modules, HTML and CSS; the
only build step is optional and generated by `kipr library site` for file:// use (below). The CI puts it next to the render
and checks output, then ships the folder as a GitHub Actions artifact (or publishes it on GitHub Pages):
    ...
    OUT/index.html, viewer.css, js/            (this directory, copied by kipr library site)
    OUT/data.js, js/bundle.js, offline/, serve.py, README.txt     (file:// support, generated by kipr library site)

```sh
kipr library site --out cr-out     # --no-offline: skip the file:// support
...
|---|---|---|
| app code | `js/app.js` as ES modules (unchanged) | `js/bundle.js`: the same modules as one classic script, generated by `kipr library site` |
...
(`tests/library/viewer/screenshot.py --mode file`), and so does the no-Worker main-thread fallback. Firefox was not tested here;
...
- `tests/library/viewer/xss_check.py` injects HTML, `javascript:` URLs and path traversal into every manifest/review field and
...
The tests live in `tests/library/` (see `docs/library.md`). By hand, from the repository root:

```sh
# realistic mock OUT (renders real footprints, synthesises "modified" bases) from the fixture repo
# (or a kicad-libs checkout)
python3 tests/library/fixture_repo.py /tmp/fx
python3 tests/library/viewer/make_mock.py --repo /tmp/fx --out /tmp/mock-out [--stock-3d DIR_WITH_KICAD_STOCK_STEPS]
kipr library site --out /tmp/mock-out
...
python3 tests/library/viewer/screenshot.py --site /tmp/mock-out --shots /tmp/shots [--dark]
python3 tests/library/viewer/screenshot.py --site /tmp/mock-out --shots /tmp/shots-file --mode file    # from disk, like an artifact
python3 tests/library/viewer/screenshot.py --site /tmp/mock-out --shots /tmp/shots-serve --mode serve  # through the site's serve.py
python3 tests/library/viewer/xss_check.py --site /tmp/mock-out [--mode file]
node --test tests/library/viewer/unit.test.mjs        # transform maths, URL filters, pad/pin row diff
python -m pytest tests/library/test_library_site.py   # bundler, data.js escaping, pack path confinement
```

`tests/library/viewer/review.mock.json` is a hand-written review in the contract's format.
