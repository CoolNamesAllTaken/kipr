"""Build ONE self-contained HTML report from a component-review output directory.
    ...
    kipr library report --out cr-out [--output FILE] [--max-mb 20]
...
and only if they pass the same active-content check as kipr.library.ci.sanitize_site. All text from
...
(kipr's dependencies) when installed, and degrades without them.
"""
...
def build(site: Path, level: int = 0, server: str = "https://github.com", now: str | None = None) -> str:
    ...
    h.append(f"Generated by kipr library report at {esc(stamp)}. "
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(prog="kipr library report", description=__doc__.split("\n\n")[0])
    add_arguments(ap)
